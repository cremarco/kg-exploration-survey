...
const MainMethodStackedChart = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const methodTypeCounts = d3.rollup(
      data,
      v => v.length,
      d => d['mainMethod']?.type || 'N/A'
    );
    const methodTypes = Array.from(methodTypeCounts.keys())
      .sort((a, b) => {
        const diff = (methodTypeCounts.get(b) || 0) - (methodTypeCounts.get(a) || 0);
    ...
    const color = d3.scaleOrdinal()
      .domain(methodTypes)
      .range(d3.quantize(d3.interpolateTurbo, methodTypes.length));
