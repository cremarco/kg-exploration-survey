...
Existing surveys cover Linked Data visualization and exploration systems (\eg Dadzie and Rowe~\cite{2011_Dadzie_Approaches_to}; Marie and Gandon~\cite{2014_Marie_Survey_of}; Jacksi et al.~\cite{2016_Jacksi_State_of}), visual query builders, and question answering. However, fewer works focus specifically on the combination of \emph{interactive guidance}, \emph{recommendations}, and \emph{multi-modal visualizations} that characterize KG exploration as an iterative sense-making process. This survey explicitly relates interaction paradigms to backend assumptions and evaluation practices.
...
\paragraph{Selection.} We selected ten representative works (2008--2022) as anchors for the taxonomy, covering faceted browsing, relationship discovery, graph navigation, guided query building, visualization recommendation, and user guidance. For completeness with respect to the local corpus collected in \texttt{paper/}, Section~\ref{sec:selected-works} and Table~\ref{tab:comparazione} also summarize additional tools and papers.
...
\label{sec:taxonomy}
We characterize approaches along the following attributes; Table~\ref{tab:taxonomy} summarizes typical design choices and Table~\ref{tab:comparazione} instantiates them for the local corpus (\texttt{paper/}).
\begin{itemize}
